...
from Hydra.database.coins import get_coins_from_users, add_coins_to_db
...
START_TEXT = "<b>{name}</b>, Am I Hydra Game Bot I've Many Games In My Sides. Let's Start Playing? Tap /help"
      ...
      if message.chat.type == enums.ChatType.PRIVATE:
             ...
             try:
                 if bool(message.text.split(None,1)[1] in TOKEN_LIST):
                       coins = await get_lottery_coins(code=message.text.split(None,1)[1], user_id=user_id)
                       if coins != False:
             ...
             if not user_id in (await get_users_list()):
                    ...
                    return await message.reply_text("You Has Been Added To My Database, That Case You Got 100 coins 💰.") 
             else:
                 return await message.reply_text(" ")
