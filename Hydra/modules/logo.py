...
from Hydra import OWNER_ID, tbot
...
async def lego(event):
    ...
    if event.sender_id != OWNER_ID and not quew:
        ...
        return
    pesan = await event.reply("`Logo Making please wait.....`")
    try:
        ...
        await tbot.send_file(
            event.chat_id, file=fname, caption="**Logo Generated by @Hydra_100_Bot**"
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception:
        await event.reply("Unknown Error Please Ask Here @FutureCity005")


__help__ = """
*Logo:* 
• /logo <name> Wait 5 sec To get Your Logo 
(Only Works Group). If Show Any Errors Ask Here @FutureCity005 .
"""
__mod_name__ = "Logo"
