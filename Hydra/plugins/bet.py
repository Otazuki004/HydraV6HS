...
from Hydra.database.level import add_level_to_db, get_users_level, level_system
...
@bot.on_message(filters.command("bet", prefixes=prefix) & filters.user(DEV_USERS))
async def bet(_, message): 
    ...
    try:
          ...
          return await message.reply_text("🆘 Example: /bet 100, Use @Hydra_100_Bot", quote=True)
    ...
    if coins_balance > coins_spend or coins_balance == coins_spend:
        ...
        if key.casefold() == "lose":
              ...
              await message.reply_text(f"🚫 You Lose {coins_spend}. Your Current coins Balance `{coins}`, Use @Hydra_100_Bot")
              ...
              return 
               ...
               coins = await get_coins_from_users(user_id)
               return await message.reply_text(f"🎊 Pro Bet🎊. 🎊 You Won {won_coins}, ✨ Your Current coins Balance `{coins}`, Use @Hydra_100_Bot", quote=True)
